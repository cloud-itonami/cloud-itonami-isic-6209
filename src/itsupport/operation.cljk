(ns itsupport.operation
  "OperationActor — one route/disclosure/dispute operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (TicketRouter-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the TicketGovernor (:govern) and the rollout
  phase gate (:decide) before anything commits to the SSoT or is
  disclosed.
  ...
  One graph run = one operation. Human-in-the-loop = real approval/dispute
  review: `interrupt-before #{:request-approval}` pauses the actor and
  hands the decision to a human reviewer (support manager)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [itsupport.llm :as llm]
            [itsupport.policy :as policy]
            [itsupport.phase :as phase]
               ...
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:correction? verdict) :dispute-request
                                          (:sla-breach-imminent? verdict) :sla-breach-imminent
                                          :else :low-confidence))
                        :phase ph
